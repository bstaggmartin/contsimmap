...
#6/9/2025: starting to implement more "robust" likelihood function that ignores outliers...
...
make.lik.fun<-function(tree,trait.data,
                       ...
                       ELBO.p=NA,
                       robust=FALSE,
                       var.cutoff=2*log(2*Nnode(tree,internal.only=FALSE)[1]),
                       n.var.cutoff=0.5,
  ...
  #so I believe this function is still assuming single topology based on the above line
  #which means there's no need to deal with multiple var.cutoffs, n.var.cutoffs, etc.
  
  var.cutoff<- -var.cutoff
  n.var.cutoff<-round(n.var.cutoff*Nnode(tree.info[["tree"]][[1]],internal.only=FALSE))
  ...
  #now keeping track of zpz terms that sum in r separately
  #allows for outlier detection/removal
  #actually no real need to hold zpz since it's just a copy of RR holder...
  #but keeping it for coding convenience for now
  ZPZ.holder<-RR.holder<-numeric(nsim)
  ...
  if(mult.traits){
    ...
    calc.obs<-function(e,i,n,nobs,yvar,in.ycor,inf,des.inf,base.ycor){
      ...
      out.ZPZ<- -.colSums(as.vector(tmp.z)*tmp.PZ,ntraits,tmp.n)
      ...
      if(nobs>1){
        for(j in seq_len(nobs)[-1]){
          ...
          out.ZPZ<-out.ZPZ-.colSums(as.vector(tmp.z)*tmp.PZ,ntraits,tmp.n)
          
          
          out.det<-out.det+det[[tmp.match]]
        }
      }
      #if tmp.n is 1, should be fine because it will get replicated when inserting into larger array
      list(out.P,out.PZ,out.det,in.ycor,out.ZPZ)
    }
    ...
    calc.all<-function(e,xvar,yvar,cors,mu,VV,ZZ,RR,double.RR,
                       ...
                       RR.holder,PZ.holder,VV.holder,des.inf.holder,ZPZ.holder){
          ...
          if(has.des[e]){
            ...
            for(d in des[[e]]){
              ...
              if(tmp.code2!=non.code){
                ...
                ZPZ.holder[tmp.inds]<-ZPZ.holder[tmp.inds]-.colSums(tmp.ZZ*tmp.PZ,tmp.k,tmp.n)
      ...
      ZPZ.holder<-ZPZ.holder+.colSums(ZZ*PZ.holder,ntraits,nsim)
      #need to make sure to count up number of var cutoffs eventually...just do this for now
      if(robust){
        RR.holder<-ZPZ.holder+RR.holder
        pen.inds<-var.cutoff>ZPZ.holder
        RR.holder[pen.inds]<-sqrt(var.cutoff/ZPZ.holder[pen.inds])*RR.holder[pen.inds]
      }else{
        RR.holder<-ZPZ.holder+RR.holder
      }
    ...
    calc.all<-function(e,xvar,yvar,cors,mu,VV,ZZ,RR,double.RR,
      ...
      if(any(tmp.inds)){
        ...
        if(single.yvar){
          ...
          ZPZ.holder<- -(sum.sq.obs[e,]*tmp.yvar)[traitID]
          RR.holder<-(tmp.nobs*log.yvar-tmp.nobs*log(2*pi))[traitID]
          if(has.obs.inf|has.des.inf){
            ZPZ.holder[obs.inf|des.inf.holder]<-0
            RR.holder[obs.inf|des.inf.holder]<-0
          }
        }else{
          VV.holder[tmp.inds2]<-tmp.nobs*tmp.yvar
          PZ.holder[tmp.inds2]<-sum.obs[e,tmp.inds3]*tmp.yvar
          ZPZ.holder[tmp.inds2]<- -sum.sq.obs[e,tmp.inds3]*tmp.yvar
          RR.holder[tmp.inds2]<-tmp.nobs*log.yvar-tmp.nobs*log(2*pi)
          if(has.obs.inf|has.des.inf){
            ZPZ.holder[obs.inf|des.inf.holder]<-0
            RR.holder[obs.inf|des.inf.holder]<-0
          }
      ...
      if(has.des[e]){
        ...
        for(d in des[[e]]){
          ...
          if(any(tmp.inds)){
            ...
            ZPZ.holder[tmp.inds]<-ZPZ.holder[tmp.inds]-tmp.ZZ*tmp.PZ
            RR.holder[tmp.inds]<-RR.holder[tmp.inds]+log(tmp.VV)
      ...
      ZPZ.holder[tmp.inds]<-ZPZ.holder[tmp.inds]+ZZ[tmp.inds]*PZ.holder[tmp.inds]
      
      
      RR.holder[tmp.inds]<-RR.holder[tmp.inds]+log.VV
  ...
  int.out<-function(par,tree.inds=NULL){
    ...
    if(!is.null(tree.inds)&conmaps.flag){
      ...
      ZPZ.holder<-RR.holder<-numeric(nsim)
      ZPZ<-RR<-rep(list(RR.holder),nedges)
      if(mult.traits){
        PZ.holder<-matrix(0,ntraits,nsim)
        ZZ<-rep(list(PZ.holder),nedges)
        VV.holder<-array(0,c(ntraits,ntraits,nsim))
    ...
    if(ELBO){
      #so I tried to avoid this...but it is a real thing so maybe just try?
      #can basically be thought of as "worst-case scenario lik"
      if(is.na(ELBO.p)){
        out.lik<-mean(LL)
      }else{
        out.lik<-quantile(LL,prob=ELBO.p)
      }
    }else{
...
#'
#' @export
make.lik.fun2<-function(tree,trait.data,
                       Xvar=NULL,Xcor=NULL,
                       mu=NULL,
                       Yvar=NULL,Ycor=NULL,
                       nsim=NULL,
                       ...
                       wgts=1,wgt.by.nobs=TRUE,
                       robust=FALSE,
                       var.cutoff=2*log(2*Nnode(tree)[1]),
                       n.var.cutoff=0.5,
                       outlier.cutoff=0,winsorize=TRUE,
                       tree.prior.exp=1,tree.balance.prior=FALSE,
                       vartol=sqrt(.Machine$double.eps),
  ...
  n.var.cutoff<-round(n.var.cutoff*Nnode(tree.info[["tree"]][[1]]))
