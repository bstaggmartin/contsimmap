functions {
  ...
  real prune(vector alpha, vector beta, vector gamma, // parameters
             ...
             int[] n_dim, int[] ind_dim, int[] pos_dim, int[] code_obs, int[] code_tip, int[] code_nod){
               ...
               if(dims(gamma)[1]){
                 ...
                 r = t .* (exp(U * alpha) .* inv_logit(W * beta));
               }
               ...
               for(i in 1:N){
                 int ind_nod[T];
                 ind_nod = ind_tip[, i];
                 X[ind_nod] = rep_vector(Y[1, i], T);
                 P[ind_nod] = rep_vector(-1, T);
                 R[ind_nod] = rep_vector(0, T);
               }
...
parameters {
  ...
  vector[A] alpha; // rate maximum parameters ("scalars")
  vector[B] beta; // rate variation parameters ("slopes")
  vector[G] gamma; // rate minimum parameters ("offsets")
