...
# tmp<-cbind(exp(2.3),
#            exp(1+1*xx)+exp(-1),
#            exp(3+0*xx-2*xx^2)+exp(-1),
#            exp(3)*plogis(0+4*xx)+exp(-1))
...
formulae<-setNames(c(r~e*exp(2.3),
                     r~e*(exp(1+1*x)+exp(-2)),
                     r~e*(exp(2.5+2*x-2*x^2)+exp(-2)),
                     r~e*(exp(3)*plogis(-1+4*x)+exp(-2))),
                   func)
...
saveRDS(df,paste0('D:/contsimmap_sim_study/sim_study_',paste(ntax,collapse='_'),'_',Sys.Date()))
