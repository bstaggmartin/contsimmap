functions {
  ...
  real prune(real intra_var, real[] inter_var, // parameters
...
parameters {
  ...
  real<lower = 0> intra_var; // intra-tip variances
  
  // inter-tip stuff (sometimes termed "evolutionary")
  real<lower = 0> unfixed_intervar[S - 1]; // unifixed rates for other states
  // rate for first state will always be set to 1 to prevent unidenfitiability
  
  // rate scalar stuff
  real<lower = 0> a; // height of peak of rate scalar-explanatory trait relationship
  real<lower = 0> b; // "narrowness" of peak of rate scalar-explanatory trait relationship
  real<lower = 0> c; // base of peak of rate scalar-explanatory trait relationship
  real d; // location of peak of rate scalar-explanatory trait relationship
  
}

transformed parameters {
  
  real inter_var[S]; // compete inter-tip variance vectors
  vector[sum(n_inc)] r; // rate scalars
  
  inter_var[1] = 1;
  if(S > 1){
    inter_var[2:S] = unfixed_intervar;
  }
  
  r = t .* (a * exp(-b / 2 * square(z - d)) + c);
...
model {
  ...
  target += prune(intra_var, inter_var,
                  N, K, S, T, E, C,
                  n_obs, ind_obs, Y,
                  n_inc, pos_inc, r, s, 
